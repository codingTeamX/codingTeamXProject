...
{
    public class Contrato:Ventas.Datos.Contexto
    {
        ...
        public DataTable datosContrato()
        {
            try
            {
        ...
        public DataTable datosNroContrato(string nombre)
        {
            try
            {
                string sql = "select * from #tabla# c where nroContrato like '" + nombre + "%'";
                sql = sql.Replace("#tabla#", tabla);
                return traerDatos(sql);
            }
            catch (Exception e)
        ...
        public int actualizar(OleDbTransaction tr)
        {
            try
            {
                string sql = "update #tabla# set nroContrato='#v1#' where id='#v0#'";
                sql = sql.Replace("#tabla#", tabla);
