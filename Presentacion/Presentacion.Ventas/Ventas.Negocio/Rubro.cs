...
{
    public class Rubro:Ventas.Datos.Contexto
    {
        string tabla = "rubro";
        public string ERROR
        {
        ...
        public string llave(OleDbTransaction tr)
        {
            try
            {
                string sql = "select max(id) from rubro";
        ...
        public DataTable datosRubroNombre(string nombre)
        {
            try
            {
                string sql = "select * from #tabla# where nombreRubro like '" + nombre + "%'";
                sql = sql.Replace("#tabla#", tabla);
                return traerDatos(sql);
            }
            catch (Exception e)
        ...
        public int actualizar(OleDbTransaction tr)
        {
            try
            {
                string sql = "update #tabla# set nombreRubro='#v1#' where id='#v0#'";
                sql = sql.Replace("#tabla#", tabla);
