...
namespace Ventas.Negocio
{
    public class Grasa:Ventas.Datos.Contexto
    {
        ...
        public string llave(OleDbTransaction tr)
        {
            try
            {
                string sql = "select max(id) from Grasa";
        ...
        public DataTable datosGrasaNombre(string nombre)
        {
            try
            {
                string sql = "select * from #tabla# c where nombreGrasa like '" + nombre + "%'";
                sql = sql.Replace("#tabla#", tabla);
                return traerDatos(sql);
            }
            catch (Exception e)
        ...
        public int actualizar(OleDbTransaction tr)
        {
            try
            {
                string sql = "update #tabla# set nombreGrasa='#v1#' where id='#v0#'";
                sql = sql.Replace("#tabla#", tabla);
