...
namespace Ventas.Negocio
{
    public class DetalleCarbohidrato : Ventas.Datos.Contexto
    {
        ...
        public DataTable datosDetalleCarbohidrato()
        {
            try
            {
        ...
        public string llave(OleDbTransaction tr)
        {
            try
            {
                string sql = "select max(id) from detalleCarbohidrato";
        ...
        public int actualizar(OleDbTransaction tr)
        {
            try
            {
                string sql = "update #tabla# set cantidad='#v2#' where idProducto='#v0#' and idCarbohidrato='#v1#'";
                sql = sql.Replace("#tabla#", tabla);
        ...
        public int eliminar(OleDbTransaction tr)
        {
            try
            {
                string sql = "delete from #tabla# where idProducto='#v0#' and idCarbohidrato='#v1#'";
                sql = sql.Replace("#tabla#", tabla);
                sql = sql.Replace("#v0#", vdatos[0]);
                sql = sql.Replace("#v1#", vdatos[1]);
                return modificar(sql, tr);
