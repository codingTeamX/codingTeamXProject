...
namespace Ventas.Negocio
{
    public class DetalleProteina:Ventas.Datos.Contexto
    {
        ...
        public DataTable datosDetalleProteina()
        {
            try
            {
        ...
        public string llave(OleDbTransaction tr)
        {
            try
            {
                string sql = "select max(id) from detalleProteina";
        ...
        public int actualizar(OleDbTransaction tr)
        {
            try
            {
                string sql = "update #tabla# set cantidad='#v2#' where idProducto='#v0#' and idProteina='#v1#'";
                sql = sql.Replace("#tabla#", tabla);
        ...
        public int eliminar(OleDbTransaction tr)
        {
            try
            {
                string sql = "delete from #tabla# where idProducto='#v0#' and idProteina='#v1#'";
                sql = sql.Replace("#tabla#", tabla);
                sql = sql.Replace("#v0#", vdatos[0]);
                sql = sql.Replace("#v1#", vdatos[1]);
                return modificar(sql, tr);
