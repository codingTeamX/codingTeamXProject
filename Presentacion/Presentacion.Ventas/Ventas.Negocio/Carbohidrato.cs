...
namespace Ventas.Negocio
{
    public class Carbohidrato : Ventas.Datos.Contexto
    {
        ...
        public DataTable datosCarbohidrato()
        {
            try
            {
        ...
        public string llave(OleDbTransaction tr)
        {
            try
            {
                string sql = "select max(id) from carbohidrato";
        ...
        public DataTable datosCarbohidratoNombre(string nombre)
        {
            try
            {
                string sql = "select * from #tabla# c where nombreCarbohidrato like '" + nombre + "%'";
                sql = sql.Replace("#tabla#", tabla);
                return traerDatos(sql);
            }
            catch (Exception e)
        ...
        public int actualizar(OleDbTransaction tr)
        {
            try
            {
                string sql = "update #tabla# set nombreCarbohidrato='#v1#' where id='#v0#'";
                sql = sql.Replace("#tabla#", tabla);
