...
{
    public class DetalleVenta:Ventas.Datos.Contexto
    {
        string tabla = "detalleVenta";
        public string ERROR
        {
            get { return traducir(error); }
        }

        public DataTable datosDetalleVenta()
        {
            try
            {
        ...
        public DataTable datosDetalleVentaID(string venta)
        {
            try
            {
                string sql = "select from venta v inner join #tabla# dv on v.id=dv.idVenta inner join producto p on p.id=dv.idProducto where v.id=" + venta;
                sql = sql.Replace("#tabla#", tabla);
                return traerDatos(sql);
            }
            catch (Exception e)
        ...
        public int insertar(OleDbTransaction tr)
        {
            try
            {
                string sql = "insert into #tabla# values('#v1#','#v2#','#v3#','#v4#')";
                sql = sql.Replace("#tabla#", tabla);
        ...
        public int actualizar(OleDbTransaction tr)
        {
            try
            {
                string sql = "update #tabla# set cantidad='#v3#' where idVenta='#v0#' and idLote='#v1#' and idProducto='#v2#'";
                sql = sql.Replace("#tabla#", tabla);
        ...
        public int eliminar(OleDbTransaction tr)
        {
            try
            {
                string sql = "delete from #tabla#where idVenta='#v0#' and idLote='#v1#' and idProducto='#v2#'";
                sql = sql.Replace("#tabla#", tabla);
                sql = sql.Replace("#v0#", vdatos[0]);
                sql = sql.Replace("#v1#", vdatos[1]);
                sql = sql.Replace("#v2#", vdatos[2]);
