...
namespace Ventas.Negocio
{
    public class Pais:Ventas.Datos.Contexto
    {
        ...
        public string llave(OleDbTransaction tr)
        {
            try
            {
                string sql = "select max(id) from pais";
        ...
        public DataTable datosPaisNombre(string nombre)
        {
            try
            {
                string sql = "select * from #tabla# where nombrePais like '" + nombre + "%'";
                sql = sql.Replace("#tabla#", tabla);
                return traerDatos(sql);
            }
            catch (Exception e)
        ...
        public int actualizar(OleDbTransaction tr)
        {
            try
            {
                string sql = "update #tabla# set nombrePais='#v1#' where id='#v0#'";
                sql = sql.Replace("#tabla#", tabla);
